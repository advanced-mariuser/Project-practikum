...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, sf::Clock &clock)
{
    ...
    const float targetAngle = toDegrees(std::atan2(delta.y, delta.x));

    const float maxRotationSpeed = 15.0f;                  // Максимальная скорость поворота в градусах в секунду
    const float elapsedTime = clock.restart().asSeconds(); // Время, прошедшее с предыдущего обновления

    float currentAngle = pointer.getRotation();
    float angleDifference = targetAngle - currentAngle;

    // Обрабатываем случай, когда разница углов больше 180 градусов
    ...
    float maxRotation = maxRotationSpeed * elapsedTime;
